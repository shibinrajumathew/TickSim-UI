/*
 * Created on: Wed Oct 07 2020 10:03:41 PM
...
/**
 *
 * @param {Number} movingAverageScale
 */
let getMAPoint = (movingAverageScale = 5) => {
  let maPoints = [];
  slicedData.map((data, index) => {
    let { high, low, open, close } = data;
    if (index < movingAverageScale) {
      maPoints = [...maPoints, null];
      return null;
    }
    ...
    for (let prevIndex = index; prevIndex >= lastIndex; prevIndex--) {
      ...
      tempAverage.low += low;
      tempAverage.open += open;
      tempAverage.close += close;
    }
    tempAverage.high /= movingAverageScale;
    tempAverage.low /= movingAverageScale;
    tempAverage.open /= movingAverageScale;
    tempAverage.close /= movingAverageScale;
    let hl = tempAverage.high + tempAverage.low;
    let hlo = hl + tempAverage.open;
    let hlc = hl + tempAverage.close;
    let hloc = hlc + tempAverage.close;
    hl /= 2;
    hlo /= 3;
    hlc /= 3;
    hloc /= 4;
