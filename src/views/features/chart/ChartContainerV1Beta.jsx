...
class ChartContainer extends Component {
  ...
  componentDidMount() {
    ...
    let candleDataFetch = setInterval(() => {
      ...
      /**
       * here last 24 elements selected and 25th element filling inside interval
       */
      data = data.slice(0).slice(-(nCandle - 1));
      let dataLength = data.length; //useful before data slice and one candle will fill on below step
      ...
      let dynamicCandleInterval = setInterval(() => {
        ...
        switch (dynamicCandleCounter) {
          case 1:
            ...
            //MV indicator
            if (data.length >= maNCandle) {
              MALastNCandles.map((candleData, index) => {
                let { high, low, open, close, date } = candleData;
                nCandlesTotal.totalHigh += high;
                nCandlesTotal.totalLow += low;
                nCandlesTotal.totalOpen += open;
                nCandlesTotal.totalClose += close;
                nCandlesTotal.date = date;
                return nCandlesTotal;
              });

              nCandlesTotalArray = [...nCandlesTotalArray, nCandlesTotal];
          ...
          case 3:
            ...
            data[dataLength] = dynamicData;
            clearInterval(dynamicCandleInterval);
            break;
          default:
            break;
        }

        dynamicCandleCounter = dynamicCandleCounter + 1;
        this.setState({
          data,
          nCandlesTotalArray,
        });
